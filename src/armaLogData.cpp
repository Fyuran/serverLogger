...
extern RVExtensionCallback callbackPtr;

namespace arma {

bool operator==(const logData& d1, const logData& d2) {
	return d1.name == d2.name && d1.uid == d2.uid && d1.fps == d2.fps &&
...
void manageSession(std::vector<String> args) {
	...
	try {
		...
		try {
			...
			if (callbackPtr)
				callbackPtr("btc_serverLogger", "btc_fnc_reportStatus", "FAIL");
		}
	}

	if (callbackPtr)
		callbackPtr("btc_serverLogger", "btc_fnc_reportStatus", "SUCCESS");
}
...
String currentDateTime(const char* fmt) {
	...
	strftime(buf, sizeof(buf), fmt,
			 &tstruct);	 // http://en.cppreference.com/w/cpp/chrono/c/strftime
...
const std::filesystem::path getCurrentFilePath(const logEntry& le) {
	std::filesystem::path path{
		std::filesystem::current_path() / "serverLogger" / le.worldName /
		currentDateTime("%Y/%m/%d") / (le.missionName + ".JSON")};
